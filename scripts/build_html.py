#!/usr/bin/env python3
"""data/rankings_us.csv と data/rankings_jp.csv から市場ごとのHTMLページを生成する。

このページの価値は「今日のランキング」ではなく「日次の横並び推移からお祭り開始銘柄を早く見つける」こと。
市場ごとに1ページ（米国株=index.html / 日本株=jp.html）を出力し、各ページには統合テーブルを1つ置く:
  最新記録日のトップ50（順位・シグナルバッジ・株価/前日比・売買代金/前回比）に、
  直近14記録日の順位ヒートマップ（緑濃淡）を右ブロックとして連結。

レイアウトはモバイル優先（銘柄列sticky＋横スクロール）だが、
デスクトップ幅ではページ幅を広げてテーブル全体を収め、横スクロールなしで読める。
日本語UI・静的HTML（外部依存なし）。出力は入力CSVに対して決定的（生成時刻等を埋め込まない）。
...
MARKETS = [
    {
        ...
        "title": "米国株",
        "page": "index.html",  # サイトのトップは米国株ページ
...
# 横スクロールを出さないためのレイアウト方針:
#   - モバイル(既定): テーブルは自然幅。銘柄列をstickyにして .tablewrap を横スクロール。
#   - デスクトップ(>=940px): body幅を広げ、テーブルを width:100% で収める。
#     銘柄列も広げて社名の省略を減らす。想定最小幅は 銘柄220 + 株価~80 + 代金~85 + 34*14 = 約860px。
STYLE = """  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  nav a { font-size:13px; color:#93c5fd; text-decoration:none; padding:7px 14px;
          background:#1a2035; border-radius:6px; border:1px solid transparent; }
  nav a:hover { background:#222a44; }
  nav a.cur { background:#1e293b; color:#f8fafc; font-weight:700;
              border-color:#3b4a68; cursor:default; }
  ...
  /* デスクトップ: ページ幅を広げてテーブルを収め、横スクロールを出さない */
  @media (min-width:940px) {
    body { max-width:1120px; }
    .merged { width:100%; table-layout:fixed; }
    .merged th.snm, .merged td.snm { width:220px; min-width:220px; max-width:220px; }
    .merged th.gd, .merged td.hc { width:auto; }
    .merged td.num, .merged th.num { width:110px; }
...
def analyze(m):
    """CSVを読み、最新日トップNの行・前回比・シグナル（テーブルのバッジ用）を計算して返す。"""
    by_date = load(m["csv"])
...
def section_html(m, a):
    """統合テーブル: 左=最新日の情報（銘柄列＋株価・代金）、右=直近HEAT_DAYS記録日の順位軌跡。"""
    if not a["dates"]:
...
右ブロックは直近{len(heat_dates)}記録日の順位軌跡（右端が最新日）。<span class="only-narrow">画面が狭いときは銘柄列を固定したまま右へスワイプできます。</span></p>
    <div class="tablewrap">
    <table class="merged">
      <thead><tr>
        <th class="snm">#・銘柄</th>
        <th class="num">株価 / 前日比</th><th class="num">売買代金 / 前回比</th>
        {head_dates}
      </tr></thead>
      <tbody>
{body}
...
def nav_html(current):
    """市場ページ間のタブ。現在のページは cur クラスでリンクしない。"""
    items = []
    for m in MARKETS:
        label = f'{m["flag"]} {m["title"]}'
        if m["key"] == current["key"]:
            items.append(f'<a class="cur" aria-current="page">{label}</a>')
        else:
            items.append(f'<a href="{m["page"]}">{label}</a>')
    return f'  <nav>{"".join(items)}</nav>'


def page_html(m, a):
    """market 1つ分の完全なHTMLページ。"""
    return f"""<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>{m["title"]} 売買代金ランキング</title>
<style>
{STYLE}</style>
</head>
<body>
  <h1>{m["flag"]} {m["title"]} 売買代金ランキング</h1>
  <p class="lead">{m["title"]}の売買代金（Value Traded）上位{TOP_N}銘柄を毎営業日に自動記録。
...
{section_html(m, a)}
  ...
  <script>
    // 画面が狭くテーブルが収まらないときだけ、軌跡の最新日（右端）が見えるようにスクロールしておく
    document.querySelectorAll(".tablewrap").forEach(function (w) {{
      if (w.scrollWidth > w.clientWidth) {{ w.scrollLeft = w.scrollWidth; }}
    }});
  </script>
</body>
</html>
...
def main():
    ...
    for m, a in results:
        out = os.path.join(ROOT, m["page"])
        with open(out, "w", encoding="utf-8") as f:
            f.write(page_html(m, a))
        print(f"{m['title']}ページを生成しました: {out}")
